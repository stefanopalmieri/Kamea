...
**Did you just encode Lisp in a lookup table?** No. We found the *space*, and Lisp is one point in it. The three capabilities (S, D, H) are structurally necessary — you need some composition, some branching, some recursion (irredundancy proved). But the specific axiom forms are not unique: composition alone admits ≥6 variants, each producing a different role assignment. The McCarthy role inventory (quote, eval, cons, car, cdr, cond, nil) is one realization — a canonical point within an equivalence class of axiom systems that all achieve the same capabilities. The architecture is forced; the specific roles are a presentation choice.
...
**What's the contribution?** A four-layer structural picture of reflective computation. (1) **Independence**: three capabilities, no one implies any other — Lean-proved in all four directions. (2) **Irredundancy**: 7 axioms across 3 capabilities, none derivable from the rest — SAT-proved. (3) **Realization freedom**: each capability admits multiple inequivalent axiom forms (≥6, ≥4, ≥3) — SAT-proved. (4) **Role relativity**: the specific role inventory depends on which realization is chosen. The capabilities are the skeleton; the axiom forms are the joints; the roles are the posture. Plus: 55 machine-checked theorems (zero `sorry`), a tight bound (N=10), and a working artifact (compiled reflective tower, 2.2 ms native).
...
- **Role uniqueness (resolved: not unique, but classified).** The three-category decomposition (absorbers, classifiers, non-classifiers) and the walls between them are proved universal. The *capabilities* (some composition, some branching, some recursion) are irredundant — you need them. But the *specific axiom forms* are not unique: Compose admits ≥6 variants, Y admits ≥4, Branch admits ≥3 — all satisfiable over S+D. Different realizations produce different role assignments within the same architecture. The McCarthy correspondence is one canonical point within an equivalence class of axiom systems, not the unique decomposition. The question is no longer "is Lisp forced?" but "what is the structure of the equivalence class of realizations?"
