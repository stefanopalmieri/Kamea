...
- **Role uniqueness (resolved: not unique, but classified).** The three-category decomposition (absorbers, classifiers, non-classifiers) and the walls between them are proved universal. The *capabilities* (some composition, some branching, some recursion) are irredundant — you need them. But the *specific axiom forms* are not unique: Compose admits ≥6 variants, Y admits ≥4, Branch admits ≥3 — all satisfiable over S+D. Different realizations produce different role assignments within the same architecture. The McCarthy correspondence is one point within an equivalence class of axiom systems, distinguished by producing the cleanest Lisp-like role separation — but not selected by any universal property. The question is no longer "is Lisp forced?" but "what is the structure of the equivalence class of realizations?"
