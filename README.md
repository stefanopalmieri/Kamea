...
*Self-simulation, self-description, and self-hosting are three independent capabilities of reflective computation. Each level requires axioms that are independent of the previous level — proved by concrete counterexamples at every boundary. A 16×16 Cayley table is the witness for all three at once, with machine-checked proofs.*
...
Lambda calculus gave functions a foundation. Turing machines gave computation a foundation. Kamea separates three independent capabilities of reflective computation — self-simulation, self-description, and self-hosting — and shows the axioms at each boundary are independent of the previous level (no level collapses into another). A 16×16 Cayley table is the witness: a finite algebra that exercises all three.

## The Three Levels

Self-simulation, self-description, and self-hosting are three independent capabilities. Each transition is *independent* of the previous level — proved by concrete finite counterexamples. Whether each transition is *minimal* (whether fewer or different axioms could achieve the same structural effect) remains open.

| Level | What's added | What it gives | Independence |
|-------|-------------|---------------|--------------|
| **Self-simulating** | Retraction pair (Q/E) | Computes own Cayley table. Partial application injectivity forced. | (base) `[Lean]` `SelfSimulation.lean` — 4 universal theorems |
| **Self-describing** | + Kripke dichotomy | Three categories (zeros, classifiers, non-classifiers) with hard walls. Judgment cannot merge with computation. | Independent of Level 1: N=8 counterexample self-simulates without dichotomy (64/64 cells) `[SAT]` |
| **Self-hosting** | + Compose + Inert | Evaluator internalized. Smith's tower terminates at 256 bytes. | Independent of Level 2: N=10 counterexamples satisfy Kripke but lack Compose (Test D) and Inert (Test E) `[SAT]` |

**The boundaries are strict.** Each level's model class is a proper subset of the previous — no level collapses into another. A retraction magma can compute its own table without the Kripke wall: a concrete 8-element counterexample with mixed elements (rows having both boolean and non-boolean outputs on the core) self-simulates perfectly. The Kripke wall is not about computing the table; it is the axiom that organizes the algebra into coherent roles. Composition and substrate are not about roles; they internalize the machine: concrete N=10 retraction magmas satisfy the Kripke dichotomy yet no element can satisfy the Compose axiom, and the algebra has no inert element.
...
The contribution is not "a small table implements Lisp." It is the **independence structure**: self-simulation, self-description, and self-hosting are three distinct capabilities, and the axioms at each boundary are independent of the previous level — proved by counterexample. Nobody has separated them before.

Self-simulation (computing your own table) requires only a retraction pair. Self-description (having coherent roles — judgment distinct from computation) additionally requires the Kripke dichotomy. Self-hosting (running the simulation without an external evaluator) additionally requires composition and substrate. Each boundary is inhabited by a concrete finite counterexample: an N=8 non-dichotomic retraction magma self-simulates but has no clean roles (Level 1 without Level 2); an N=10 dichotomic retraction magma has roles but no element satisfying Compose and no inert element (Level 2 without Level 3). The Ψ₁₆ᶠ table is all three at once.
...
**Did you just encode Lisp in a lookup table?** No. A lookup table stores data; this table *computes*. The architecture — three categories (absorbers, classifiers, non-classifiers) with hard walls between them — is proved universal: every finite dichotomic retract magma decomposes the same way, and five independent formalizations (phenomenological, information-theoretic, category-theoretic, game-theoretic, self-simulation) converge on the same categories and the same walls. That architecture is not encoded; it is a theorem. Within that architecture, the seven specific roles (quote, eval, cons, car, cdr, cond, nil) emerge from axiom interaction — nobody axiomatized "there must be a pair constructor"; the pair structure falls out of branching + substrate. Whether those seven roles are the *unique* functional decomposition within the three categories, or one of several equivalent decompositions, is open. What is proved: the architecture is inevitable; the specific role inventory is convergently natural (recovered independently by multiple axiom systems) but not proved unique.
...
**What's the contribution?** Three things. (1) The three-level independence result: self-simulation, self-description, and self-hosting are separable capabilities with independent axioms at each boundary — proved by concrete finite counterexamples (N=8 for Level 1→2, N=10 for Level 2→3). (2) Machine-checked proofs: 23 universal algebraic theorems (zero `decide`, zero `sorry`) from two independent sources — Kripke axioms and self-simulation. (3) A working artifact: a compiled reflective tower (2.2 ms native) where a program verifies its own algebraic substrate, reifies its continuation, and rewrites its own control flow.
...
The axioms correspond to the three levels: the retraction pair (level 1) enables self-simulation, the Kripke dichotomy (level 2) creates clean roles, and compose + inert (level 3) internalize the evaluator. Each level's axioms are independent of the previous level (counterexamples at N=8 and N=10). Full inevitability analysis: [`docs/inevitability_summary.md`](docs/inevitability_summary.md).
...
- **Role uniqueness.** The three-category decomposition (absorbers, classifiers, non-classifiers) and the walls between them are proved universal — every dichotomic retract magma has them. But the seven specific roles within those categories (quote, eval, cons, car, cdr, cond, substrate) are not proved to be the unique functional decomposition. Three of four tested alternative axiom systems converge on the same role inventory (2-1-8-1 distribution), which is strong convergent evidence but not a uniqueness proof. An alternative axiom set might produce different primitives within the same three categories that achieve equivalent computational power. The architecture is inevitable; the specific instantiation is convergently natural but not proved unique. Whether the McCarthy correspondence is a deep structural necessity or one of several equivalent encodings remains the central open question.
...
- **Axiom independence (resolved).** Each level boundary is inhabited by a concrete finite counterexample, establishing model-theoretic independence. (1) The **Kripke dichotomy is independent of self-simulation**: a concrete N=8 non-dichotomic retraction magma with mixed elements (rows having both boolean and non-boolean outputs on the core) self-simulates perfectly — 64/64 cells correct. The Kripke wall is not about computing the table — it is the axiom that organizes the algebra into coherent roles (classifiers vs transformers), lifting self-simulation into self-*description*. (2) **Compose is independent of Kripke**: an N=10 retraction magma satisfies the Kripke dichotomy but no element can satisfy `η·x = ρ·(g·x)`. (3) **Inert is independent of Kripke**: an N=10 retraction magma satisfies the Kripke dichotomy but every non-absorber is a tester or encoder — no substrate element exists. Whether the axioms at each boundary are *minimal* (whether fewer or different axioms could achieve the same structural effect) remains open. See [`docs/self_simulation_necessity.md`](docs/self_simulation_necessity.md).
