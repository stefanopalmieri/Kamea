...
This right-absorption property has two consequences. First, for any core $x$: since $x = r \dotop (s \dotop x)$ (retraction pair), right-absorption gives $\text{cls} \dotop x = \text{cls} \dotop (r \dotop (s \dotop x)) = \text{cls} \dotop r$. So $\text{cls}$ is constant on core at the value $c = \text{cls} \dotop r \in \{z_1, z_2\}$. Second, $\text{cls}$ must hit both $z_1$ and $z_2$ (else extensionality forces $\text{cls} = z_i$); let $b_1, b_2$ witness this. Then $\text{cls} \dotop z_1 = \text{cls} \dotop (\text{cls} \dotop b_1) = \text{cls} \dotop \text{cls} = c$ (since $\text{cls}$ is core), and symmetrically $\text{cls} \dotop z_2 = c$. So $\text{cls}$ is constant everywhere. By extensionality, $\text{cls} = z_i$, contradicting $\text{cls} \neq z_1, z_2$.
...
A $10 \times 10$ Cayley table defining a $\DRM$ satisfying the classifier dichotomy. Lean proves that no triple $(a, b, c)$ of pairwise distinct non-absorbers satisfies the $\ICP$ (Definition~\ref{def:icp}; equivalence with Compose+Inert established in Section~\ref{sec:icp}), by exhaustive search via \texttt{native\_decide}. Lean files: \texttt{Countermodels10.lean} (DRM verification), \texttt{ICP.lean} (theorem \texttt{dNotH\_no\_icp}).
...
\paragraph{Tightness.}
The bound $N\!=\!10$ is tight under the standard distinctness requirement (all role-bearing elements pairwise distinct): 10 elements need $N \geq 10$ by counting, and the Lean witness achieves it. Whether relaxing distinctness permits smaller witnesses is open.
...
\paragraph{H: categorical motivation.} The $\ICP$ was motivated by internal composition in monoidal closed categories, where the internal hom $[B, C]$ represents $\mathrm{Hom}(B, C)$ as an object. The $\ICP$ asks for a weaker condition: the left-regular representation contains at least one non-trivially composed element $L_a = L_c \circ L_b$. Full closure ($\{L_a\}$ closed under composition) fails for all tested models. Unlike S, the connection to internal hom is not formal — the $\ICP$ is a concrete algebraic property that we prove equivalent to Compose+Inert (Theorem~\ref{thm:icp-equiv}) and invariant under isomorphisms, but whether it corresponds to a standard categorical universal property remains to be established.
...
Plotkin and Power~\cite{plotkin03} decompose computational effects into algebraic operations with generic effects that classify each operation's arity — a structural parallel to our classifier dichotomy. Plotkin and Pretnar~\cite{plotkin09} show that effect handlers are algebra homomorphisms from a free model, with the return clause as section and the handler as retraction — our S is the finite-carrier instance of exactly this structure. Hyland and Power~\cite{hyland07} develop the Lawvere-theoretic framework in which our magma is a model of the free theory on one binary operation. Hyland, Levy, Plotkin, and Power~\cite{hyland06} show that the continuation monad is not algebraic; our no-associativity result (Theorem~\ref{thm:no-assoc}) is a structural parallel — the classifier dichotomy, like continuations, is incompatible with equational collapse. The algebraic effects literature studies finitary theories over infinite carriers; our contribution operates in the complementary setting of finite carriers with no equations.
...
Each capability admits multiple axiom forms — SAT search finds 6 satisfiable Compose variants for H, each producing a different classifier/non-classifier distribution. The independence result holds for all variants. Whether a canonical selection principle exists is open.

% ══════════════════════════════════════════════════════════════
\section{Conclusion}
% ══════════════════════════════════════════════════════════════

We have shown that three capabilities of reflective computation — self-simulation, self-description, and evaluator internalization — are independent in finite extensional magmas, each admitting a single-concept presentation-independent definition. The independence is machine-checked; the decomposition is invariant; and the magma setting is necessary — no $\DRM$ is associative (Theorem~\ref{thm:no-assoc}), ruling out semigroups, monoids, and groups. The classifier dichotomy is epistemic, not computational: an algebra can evaluate without understanding itself as evaluating.
...
\begin{enumerate}
  ...
  \item \emph{Minimum coexistence bound}: The true minimum for S+D+H lies in $[5, 10]$. SAT search shows $N\!=\!6$ is achievable with role overlap; the exact minimum is open.
\end{enumerate}
