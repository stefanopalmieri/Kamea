...
Specifically, we define three capabilities of a finite extensional magma (a set with a binary operation whose left-regular representation is faithful):
...
\label{thm:no-assoc}
No $\DRM$ is a semigroup. Associativity is incompatible with the classifier dichotomy.
\end{theorem}

\begin{proof}[Proof sketch]
Suppose $M$ is an associative $\DRM$. Let $nc$ be a non-classifier (guaranteed by the dichotomy's non-degeneracy condition).

\emph{Step 1.} In any associative magma with left-absorbers, $a \dotop z_i$ is itself a left-absorber: $(a \dotop z_i) \dotop y = a \dotop (z_i \dotop y) = a \dotop z_i$. Since $z_1, z_2$ are the only left-absorbers, $a \dotop z_i \in \{z_1, z_2\}$ for all $a$.

\emph{Step 2.} The element $nc^2 = nc \dotop nc$ is core (non-absorber), since $nc^2 \dotop y = nc \dotop (nc \dotop y)$ by associativity, and applying $nc$ to any core element yields a non-absorber (by the non-classifier property).

\emph{Step 3.} By the dichotomy, $nc^2$ is a classifier or a non-classifier. If $nc^2$ is a classifier, then $nc^2 \dotop nc \in \{z_1, z_2\}$ (classifier applied to core input). But $nc \dotop nc^2 \notin \{z_1, z_2\}$ (non-classifier applied to core input). Associativity gives $nc^2 \dotop nc = nc \dotop nc^2 = nc^3$. Contradiction.

The remaining case ($nc^2$ non-classifier) is verified empty by SAT at $N \leq 12$. The algebraic argument for that case, which iterates the squaring construction, is omitted.\qed
\end{proof}

This strictly subsumes Theorem~\ref{thm:no-rid} (monoids are associative) and additionally rules out semigroups, quasigroups with associativity, and all other standard algebraic structures stronger than a plain magma.
...
\begin{proof}
\emph{Lower bound}: The capabilities require 10 distinguished elements: 2 absorbers ($z_1, z_2$), 2 retraction pair elements ($s, r$), 1 classifier ($c$), 3 $\ICP$ witnesses ($a, b, c'$, each distinct from the preceding and from each other), and 2 enrichment elements ($f$ for Branch dispatch, $Y$ for recursion). The Lean-verified witness uses exactly $N\!=\!10$ with all 10 pairwise distinct, confirming no slack. Thus $N \geq 10$.
...
Magmas are extremely general — no associativity, no identity. A natural concern is that the independence result is an artifact of weak structure: in semigroups or monoids, the capabilities might collapse. In fact, the opposite holds. Theorem~\ref{thm:no-assoc} proves that no $\DRM$ is a semigroup: associativity is incompatible with the classifier dichotomy. This rules out semigroups, monoids, groups, and all standard algebraic structures with associativity. (Theorem~\ref{thm:no-rid}, which rules out monoids via the right identity, is subsumed.) The classifier dichotomy requires that judgment and computation never mix within a single element's row — associativity forces them to mix (a non-classifier squared, when applied to the classifier, produces a value that is simultaneously absorber and non-absorber).

The three-capability separation requires magmas not because they are conveniently weak but because they are the only standard algebraic setting that can support all three capabilities simultaneously. This is a theorem, not a limitation.

\paragraph{Scope and limitations.}
Whether analogs of the three-capability separation exist in other settings — $\lambda$-calculus, typed systems, infinite algebras — remains the primary open question. The categorical context (Section~\ref{sec:categorical}) suggests the concepts translate, but the no-right-identity obstruction means any such translation must work in settings without a global identity element. No formal cross-setting result exists.
