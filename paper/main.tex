...
\begin{enumerate}
  ...
  \item $s, r \in \core(S) := S \setminus \{z_1, z_2\}$ form a \emph{retraction pair}: $r \dotop (s \dotop x) = x$ and $s \dotop (r \dotop x) = x$ for all $x \in \core(S)$.
\end{enumerate}
\end{definition}
...
\begin{enumerate}
  ...
  \item $S \not\Rightarrow H$: The minimal $\DRM$ witness ($N\!=\!4$) trivially lacks $\ICP$ since $\ICP$ requires $\geq 3$ distinct non-absorber elements.
\end{enumerate}
\end{theorem}
...
A $10 \times 10$ Cayley table defining an $\FRM$ with a retraction pair, Branch, Compose ($\eta \dotop x = \rho \dotop (g \dotop x)$), Inert ($g$ preserves core), and Y fixed-point — all evaluation machinery — yet four elements (indices~5, 7, 8, 9) have mixed rows violating the dichotomy. Lean proves both that $\ICP$ holds (witnessed by $a\!=\!8, b\!=\!6, c\!=\!7$) and that the dichotomy fails. Lean file: \texttt{Countermodels10.lean}.
...
\begin{proof}
\emph{Lower bound}: S requires $\geq 2$ absorbers plus a retraction pair ($\geq 4$ elements). D adds a classifier ($\geq 5$). H ($\ICP$) adds three pairwise distinct non-absorber elements. Together with D's classifier, at least 10 distinct elements are needed: $z_1, z_2, s, r, c, a, b, c', f, y$ (where some may coincide with earlier elements but $\ICP$'s pairwise distinctness and the classifier's distinctness from non-classifiers require $\geq 10$ after accounting for all constraints). The Lean-verified minimal coexistence witness uses exactly $N\!=\!10$ with no element slack.
...
The three capabilities correspond to standard categorical concepts applied to finite extensional magmas:

\begin{center}
\begin{tabular}{lll}
\textbf{Capability} & \textbf{Categorical analog} & \textbf{Standard?} \\
\hline
S (retraction pair) & Section-retraction pair & Yes \\
D (classifier dichotomy) & Decidable subobject classifier & Novel (finite) \\
H ($\ICP$) & Partial internal composition & Yes (fragment) \\
\end{tabular}
\end{center}

The retraction pair and internal composition are standard categorical constructions. The classifier dichotomy — every morphism is either entirely truth-valued or entirely non-truth-valued on core — is a novel finite algebraic condition. It is the finite analog of \emph{decidability} of the subobject classifier: in a topos, the subobject classifier $\Omega$ partitions morphisms into $\Omega$-valued and non-$\Omega$-valued. The dichotomy asserts this partition is total (no partially-$\Omega$-valued morphisms), which is a strong finiteness-dependent condition.

The three-category decomposition ($Z$, $C$, $N$) uses standard components (zero morphisms, subobject classifier image, complement) in a novel combination. The functoriality theorem (Theorem~\ref{thm:functoriality}) shows this decomposition is an algebraic invariant, not an artifact of any particular presentation.
...
Smith's 3-Lisp~\cite{smith84} introduced the reflective tower — an infinite chain of meta-interpreters. Subsequent implementations (Black~\cite{black92}, Brown~\cite{brown94}, Blond~\cite{danvy88}) added live meta-modification. Amin and Rompf~\cite{amin18} showed how to compile through the tower via stage polymorphism. All of these systems combine self-simulation, self-description, and evaluator internalization without separating them. Our contribution is the separation and the proof that it is genuine (no capability implies any other).
...
\bibitem{black92}
Andrew P. Black.
\newblock Towards a theory of reflection for {Black}.
\newblock Technical report, 1992.

\bibitem{brown94}
Ron Brown.
\newblock Brown: A reflective tower with first-class environments.
