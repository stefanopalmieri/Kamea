...
We prove independence by exhibiting Lean-verified finite counterexamples in all four non-trivial directions ($N\!=\!8$ and $N\!=\!10$), exhibit a coexistence witness at $N\!=\!10$, prove that no $\DRM$ is associative (ruling out semigroups, monoids, and groups), prove the three-category decomposition is an algebraic invariant preserved by isomorphisms, and show the ICP is logically equivalent to the standard Compose+Inert axioms. All results are machine-checked in Lean~4, zero \texttt{sorry}.
\end{abstract}
...
All results are formalized in Lean~4 and verified by \texttt{lake build}, zero \texttt{sorry}. The proofs divide into universal algebraic theorems (proved by pure equational reasoning, no \texttt{decide}) and model-specific verifications (proved by \texttt{native\_decide} on concrete Cayley tables).
...
The $\ICP$ definition requires 3 pairwise distinct non-absorbers $(a, b, c)$, but does not require them to be distinct from the retraction pair or classifier. SAT search confirms that the capabilities coexist at $N\!=\!6$ when role overlap is permitted (the retraction pair doubles as $\ICP$ witnesses). The true minimum for S+D+H lies in $[5, 10]$; the exact value depends on which overlaps the axioms permit. The $N\!=\!10$ witness with all roles distinct serves as the reference model.
...
Magmas are extremely general — no associativity, no identity. A natural concern is that the independence result is an artifact of weak structure: in semigroups or monoids, the capabilities might collapse. Two results address this. Theorem~\ref{thm:no-rid} proves that no $\DRM$ has a right identity, ruling out monoids and groups. Theorem~\ref{thm:no-assoc} proves the stronger result: no $\DRM$ is associative, ruling out semigroups entirely. Both are Lean-proved by pure algebraic reasoning (zero \texttt{sorry}, no \texttt{decide}). Together, these results show that the magma setting is not conveniently weak — stronger algebraic structures cannot support capability D at all.

\paragraph{Scope and limitations.}
Whether analogs of the three-capability separation exist in other settings — $\lambda$-calculus, typed systems, infinite algebras — remains the primary open question. The categorical context (Section~\ref{sec:categorical}) suggests the concepts translate, but Theorems~\ref{thm:no-rid} and~\ref{thm:no-assoc} show that any such translation must work in settings without identity elements or associativity. No formal cross-setting result exists.
...
We have shown that three capabilities of reflective computation — self-simulation, self-description, and evaluator internalization — are independent in finite extensional magmas, each admitting a single-concept presentation-independent definition. The independence is machine-checked; the decomposition is invariant; and the magma setting is forced (no $\DRM$ is associative). The classifier dichotomy is epistemic, not computational: an algebra can evaluate without understanding itself as evaluating.
