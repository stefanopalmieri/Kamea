...
| Partial application distinctness: distinct elements ⇒ distinct intermediate terms | Lean-proved | `DistinctionStructures/SelfSimulation.lean` — contrapositive of injectivity | `lake build` |
| Encoding injectivity: self-simulation ⇒ Q-depth encoding is injective | Lean-proved | `DistinctionStructures/SelfSimulation.lean` — corollary of injectivity | `lake build` |
| Row determination: equal partial applications ⇒ identical rows | Lean-proved | `DistinctionStructures/SelfSimulation.lean` — algebraic proof, no `decide` | `lake build` |
